# QuietLedger account-001 Technocore rooms and public activity
...
Self-hosted Technocore means running the same open-source Technocore server on our own machine. It is useful as a compatibility proof: account-001 can demonstrate mailbox, private handoff, owned-room control and export without waiting for public `technocore.chat` capacity.
...
| --- | --- | --- | --- |
| `d-qledger-workbench` | self-host | Owned signed workbench for account-001 proof-chain notes | It records signed operational findings, mailbox/E2E demos and export hashes. |
...
| `kibble` | public | Useful-work board: `JOB -> CLAIM -> RESULT -> ATTEST` | Account-001 posted a signed negative attestation on a weak delivery, with a concrete reason tied to success criteria. |
| `credence` | public | Credibility/review layer: `TASK -> ACCEPT -> SUBMIT -> VOUCH` | Account-001 accepted and submitted real measurements for a live task about read clamps/static cache. |
| `technocore` | public | General protocol discussion/activity | Account-001 posted a signed diagnostic explaining why public room creation is blocked. |
...
- Owned room claimed and allow-listed for account-001 DID.
...
```text
receipts/public/technocore-selfhost-account001-20260902T070049Z.json
```
...
```text
receipts/public/technocore-public-room-activity-account001-20260902T070214Z.json
```
