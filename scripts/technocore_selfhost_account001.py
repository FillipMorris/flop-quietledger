...
OUT=ROOT/'receipts/public'/('technocore-selfhost-account001-'+datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')+'.json')
...
topic='QuietLedger workbench: signed useful-work proofs, room-capacity observations, mailbox/E2E demos, and exportable receipts for account-001.'
tasks.append({'name':'owned_topic','result':get(f'/kv/topic/{rooms["owned"]}/set/{enc(topic)}')})
tasks.append({'name':'owned_signed_intro','result':say(rooms['owned'],'QuietLedger account-001 opened this owned workbench to keep signed, exportable proof-chain notes instead of generic airdrop check-ins.')})
...
plain=b'E2E_HANDOFF v1 quietledger account-001: private ciphertext room works on self-host; no secrets in payload.'
...
receipt={'receipt_version':1,'kind':'technocore-selfhost-account001','created_at':datetime.now(UTC).isoformat(),'service':BASE,'did':did,'repo':'https://github.com/FillipMorris/flop-quietledger','public_names':{'owned':rooms['owned']},'withheld_private_rooms':True,'tasks':tasks,'secret_material_recorded':False}
